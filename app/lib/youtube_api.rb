...
class YoutubeApi
  ...
  def self.videos(video_ids)
    ...
    res = client.get 'videos' do |req|
      ...
      req.params[:id] = if video_ids.is_a?(Array)
                          video_ids.join(',')
                        else
                          video_ids
                        end
  ...
  def self.channels(channel_ids)
    ...
    res = client.get 'channels' do |req|
      req.params[:key] = API_KEY
      req.params[:part] = 'statistics'
      req.params[:id] = if channel_ids.is_a?(Array)
                          channel_ids.join(',')
                        else
                          channel_ids
                        end
  ...
  def self.recursive_search(search_keyword, limit = 100)
    items = []
    json = search(search_keyword)
    items.concat(json['items'])
    next_page_token = json['nextPageToken']
    while items.count < limit
      json = search(search_keyword, next_page_token)
      items.concat(json['items'])
      next_page_token = json['nextPageToken']
  ...
  def self.video_filter(items, limit_date_count, limit_play_per_subscriber_rate)
    ...
    items.each do |item|
      ...
      channel_statics = channels(channel_id)
      ...
      ap published_at
      unless (published_at + limit_date_count.days) > Time.now && limit_play_per_subscriber_rate <= play_per_subscriber_rate
        next
      end
